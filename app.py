...
if st.sidebar.button("開始亞當理論分析"):
    with st.spinner("讀取數據與計算亞當映射中..."):
        ...
        if df.empty or len(df) < reflection_days:
            ...
            # 取出最近 N 天的歷史價格
            recent_prices = close_s.tail(reflection_days).values
            last_price = recent_prices[-1]
            last_date = df.index[-1]

            # 第一次映射 (上下翻轉 Delta) -> 第二次映射 (時間前後翻轉)
            # 數學等價公式：Future_Price[i] = Last_Price + (Last_Price - Historical_Price[N - 1 - i])
            reversed_historical = recent_prices[::-1]
            future_adam_prices = last_price + (last_price - reversed_historical)

            # 產生未來的交易日（排除假日簡化版：按工作日推進）
            future_dates = pd.date_range(start=last_date + pd.Timedelta(days=1), periods=reflection_days, freq='B')
