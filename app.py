...
if st.sidebar.button("開始亞當理論分析"):
    with st.spinner("讀取最新數據與計算亞當映射中..."):
        ...
        if df.empty or len(df) < reflection_days:
            ...
            # 2. 剔除第一個點 (reversed_historical[0] 為當天價)，讓映射從次日動能開始，解決同價問題
            future_adam_prices = last_price + (last_price - reversed_historical[1:])

            # 3. 產生未來交易日 (數量與修正後的 future_adam_prices 保持一致)
            next_start_date = last_date + pd.Timedelta(days=1)
            future_dates = pd.date_range(
                start=next_start_date, 
                periods=len(future_adam_prices) * 2, 
                freq='B'
            )[:len(future_adam_prices)]
            ...
            col2.metric(
                f"未來 {len(future_adam_prices)} 日目標價",
            ...
            # --- [Part 4: 顯示預測數據表] ---
            st.subheader(f"🔮 未來 {len(future_adam_prices)} 個交易日軌跡數據")
            st.dataframe(
